// Variabel Global
let state = 'menu';
const c = 299792458;
let v = 0, gamma = 1, t0 = 10, t = 10, L0 = 100, L = 100;
let stars = [];
let rocketX = 0;

// Quiz
let soal = [
  ['Waktu di roket cepat akan:', ['Lebih cepat', 'Lebih lambat', 'Tetap sama', 'Berhenti'], 1],
  ['Rumus Lorentz adalah:', ['γ=1/√(1-v²/c²)', 'γ=√(1-v²/c²)', 'γ=v/c', 'γ=c/v'], 0],
  ['Benda cepat terlihat:', ['Lebih panjang', 'Lebih pendek', 'Tetap sama', 'Menghilang'], 1],
  ['Penemu Relativitas:', ['Newton', 'Einstein', 'Galileo', 'Hawking'], 1]
];
let no = 0, skor = 0, jawab = false, ans = -1;

function setup() {
  createCanvas(900, 650);
  textFont('Arial');
  for (let i = 0; i < 150; i++) {
    stars.push({x: random(width), y: random(height), size: random(1, 3), speed: random(0.5, 2)});
  }
}

function draw() {
  background(10, 10, 30);
  
  // Bintang bergerak
  for (let star of stars) {
    fill(255, 255, 255, random(150, 255));
    noStroke();
    circle(star.x, star.y, star.size);
    star.x -= star.speed * (1 + v * 2); // Bergerak sesuai kecepatan
    if (star.x < 0) {
      star.x = width;
      star.y = random(height);
    }
  }
  
  if (state === 'menu') menu();
  else if (state === 'simulasi') simulasi();
  else if (state === 'quiz') quiz();
}

// ===== MENU =====
function menu() {
  fill(100, 200, 255);
  textSize(52);
  textAlign(CENTER, CENTER);
  text('🚀 RELATIVITAS KHUSUS', width/2, 100);
  
  fill(200, 220, 255);
  textSize(22);
  text('Time Dilation & Length Contraction', width/2, 150);
  
  fill(20, 30, 60, 240);
  stroke(100, 150, 255);
  strokeWeight(3);
  rect(70, 200, width - 140, 240, 12);
  
  noStroke();
  fill(255, 255, 100);
  textSize(22);
  textAlign(LEFT);
  text('📚 Konsep Fisika Kelas 12:', 100, 240);
  
  fill(220, 220, 255);
  textSize(17);
  text('• Dilatasi Waktu: t = γ·t₀', 110, 280);
  text('   Waktu melambat pada objek cepat', 110, 305);
  text('• Kontraksi Panjang: L = L₀/γ', 110, 340);
  text('   Panjang menyusut searah gerak', 110, 365);
  text('• Faktor Lorentz: γ = 1/√(1-v²/c²)', 110, 400);
  
  btn(width/2 - 220, 480, 200, 65, '🔬 SIMULASI', color(60, 150, 255));
  btn(width/2 + 20, 480, 200, 65, '🎯 KUIS', color(255, 150, 60));
  
  fill(150, 150, 200);
  textSize(13);
  textAlign(CENTER);
  text('Fisika SMA/SMK Kelas 12 - Einstein 1905', width/2, height - 15);
}

// ===== SIMULASI =====
function simulasi() {
  fill(255);
  textSize(28);
  textAlign(CENTER, CENTER);
  text('🔬 Simulasi Interaktif', width/2, 40);
  
  fill(20, 30, 60, 240);
  stroke(100, 150, 255);
  strokeWeight(3);
  rect(25, 65, width - 50, 85, 10);
  
  noStroke();
  fill(255, 200, 100);
  textSize(16);
  textAlign(LEFT);
  text('⚡ Kecepatan Roket:', 45, 90);
  
  fill(50, 50, 80);
  rect(45, 105, width - 90, 15, 7);
  
  if (mouseIsPressed && mouseY > 100 && mouseY < 125 && mouseX > 45 && mouseX < width - 45) {
    v = constrain(map(mouseX, 45, width - 45, 0, 0.99), 0, 0.99);
  }
  
  fill(100, 200, 255);
  rect(45, 105, map(v, 0, 0.99, 0, width - 90), 15, 7);
  fill(255);
  circle(map(v, 0, 0.99, 45, width - 45), 112.5, 20);
  
  fill(255, 255, 100);
  textSize(14);
  textAlign(CENTER);
  text((v * 100).toFixed(1) + '% c', width/2, 135);
  
  gamma = 1 / sqrt(1 - v * v);
  t = gamma * t0;
  L = L0 / gamma;
  
  fill(20, 30, 60, 240);
  stroke(255, 150, 150);
  strokeWeight(3);
  rect(25, 165, (width - 60) / 2, 115, 10);
  
  noStroke();
  fill(255, 150, 150);
  textSize(18);
  text('⏱️ DILATASI WAKTU', 25 + (width - 60) / 4, 190);
  
  fill(255);
  textSize(14);
  textAlign(LEFT);
  text('γ = ' + gamma.toFixed(3), 45, 220);
  text('Waktu Roket: ' + t0 + ' jam', 45, 245);
  fill(255, 100, 100);
  text('Waktu Bumi: ' + t.toFixed(2) + ' jam', 45, 265);
  
  fill(20, 30, 60, 240);
  stroke(150, 255, 150);
  strokeWeight(3);
  rect(width/2 + 5, 165, (width - 60) / 2, 115, 10);
  
  noStroke();
  fill(150, 255, 150);
  textSize(18);
  textAlign(CENTER);
  text('📏 KONTRAKSI PANJANG', width/2 + 5 + (width - 60) / 4, 190);
  
  fill(255);
  textSize(14);
  textAlign(LEFT);
  text('L₀ = ' + L0 + ' m', width/2 + 25, 220);
  fill(255, 100, 100);
  text('L = ' + L.toFixed(2) + ' m', width/2 + 25, 245);
  fill(255, 255, 100);
  text('Rasio: ' + ((L/L0) * 100).toFixed(1) + '%', width/2 + 25, 265);
  
  fill(10, 10, 30, 250);
  stroke(100, 150, 255);
  strokeWeight(3);
  rect(25, 295, width - 50, 285, 10);
  
  noStroke();
  fill(255, 200, 100);
  textSize(17);
  textAlign(CENTER);
  text('🌌 Visualisasi', width/2, 320);
  
  fill(150, 150, 200);
  textSize(13);
  text('Roket Diam (L₀ = ' + L0 + ' m)', width/2, 355);
  roketStatis(width/2, 380, L0, color(100, 100, 200, 180));
  
  fill(255, 150, 150);
  text('Roket Bergerak (L = ' + L.toFixed(1) + ' m)', width/2, 430);
  
  rocketX += v * 3;
  if (rocketX > width + 100) rocketX = -100;
  
  roketBergerak(rocketX, 455, L, color(255, 100, 100));
  
  jam(130, 530, 32, t0, 'Roket', color(100, 255, 100));
  jam(width - 130, 530, 32, t, 'Bumi', color(255, 100, 100));
  
  btn(25, 595, 100, 35, 'Reset', color(200, 80, 80));
  btn(width - 125, 595, 100, 35, '← Menu', color(100, 100, 150));
}

// ===== QUIZ =====
function quiz() {
  fill(255);
  textSize(32);
  textAlign(CENTER, CENTER);
  text('🎯 Kuis', width/2, 50);
  
  fill(255, 255, 100);
  textSize(17);
  text('Soal ' + (no + 1) + '/' + soal.length + ' | Skor: ' + skor, width/2, 85);
  
  if (no < soal.length) {
    let s = soal[no];
    
    fill(20, 30, 60, 240);
    stroke(100, 150, 255);
    strokeWeight(3);
    rect(40, 115, width - 80, 70, 10);
    
    noStroke();
    fill(255);
    textSize(18);
    text(s[0], width/2, 150);
    
    for (let i = 0; i < 4; i++) {
      let y = 205 + i * 70;
      let hover = mouseX > 70 && mouseX < width - 70 && mouseY > y && mouseY < y + 55;
      
      if (jawab) {
        if (i === s[2]) fill(100, 255, 100);
        else if (i === ans) fill(255, 100, 100);
        else fill(40, 40, 80);
      } else {
        fill(hover ? color(80, 120, 200) : color(40, 60, 120));
        if (hover) cursor(HAND);
      }
      
      stroke(100, 150, 255);
      strokeWeight(3);
      rect(70, y, width - 140, 55, 10);
      
      noStroke();
      fill(255);
      textSize(16);
      textAlign(LEFT, CENTER);
      text(String.fromCharCode(65 + i) + '. ' + s[1][i], 95, y + 27);
    }
    
    if (jawab) {
      btn(width/2 - 80, 540, 160, 45, no < soal.length - 1 ? 'Lanjut →' : '📊 Hasil', color(60, 150, 255));
    }
  } else {
    fill(20, 30, 60, 240);
    stroke(100, 255, 100);
    strokeWeight(4);
    rect(140, 130, width - 280, 280, 15);
    
    noStroke();
    let p = (skor / soal.length) * 100;
    
    if (p >= 75) {
      fill(100, 255, 100);
      textSize(38);
      text('🎉 HEBAT!', width/2, 185);
    } else if (p >= 50) {
      fill(255, 255, 100);
      textSize(38);
      text('👍 BAGUS!', width/2, 185);
    } else {
      fill(255, 150, 150);
      textSize(38);
      text('📚 COBA LAGI', width/2, 185);
    }
    
    fill(255);
    textSize(28);
    text('Skor: ' + skor + '/' + soal.length, width/2, 250);
    fill(255, 255, 100);
    textSize(24);
    text('Nilai: ' + p.toFixed(0) + '%', width/2, 290);
    
    fill(200, 200, 255);
    textSize(15);
    text('Terus belajar Fisika!', width/2, 340);
    
    btn(width/2 - 175, 460, 140, 45, '🔄 Ulangi', color(255, 150, 60));
    btn(width/2 + 35, 460, 140, 45, '← Menu', color(100, 100, 150));
  }
}

// ===== HELPER =====
function btn(x, y, w, h, txt, col) {
  let hover = mouseX > x && mouseX < x + w && mouseY > y && mouseY < y + h;
  fill(hover ? color(col.levels[0] + 40, col.levels[1] + 40, col.levels[2] + 40) : col);
  cursor(hover ? HAND : ARROW);
  stroke(255);
  strokeWeight(3);
  rect(x, y, w, h, 10);
  fill(255);
  noStroke();
  textSize(16);
  textAlign(CENTER, CENTER);
  text(txt, x + w/2, y + h/2);
}

function roketStatis(x, y, len, col) {
  push();
  translate(x, y);
  let s = len / L0;
  fill(col);
  stroke(255);
  strokeWeight(2);
  rect(-50 * s, -13, 100 * s, 26, 5);
  triangle(50 * s, -13, 50 * s, 13, 67 * s, 0);
  fill(100, 200, 255);
  circle(0, 0, 11);
  fill(col);
  triangle(-50 * s, -13, -60 * s, -22, -50 * s, -2);
  triangle(-50 * s, 13, -60 * s, 22, -50 * s, 2);
  pop();
}

function roketBergerak(x, y, len, col) {
  push();
  translate(x, y);
  let s = len / L0;
  fill(col);
  stroke(255);
  strokeWeight(2);
  rect(-50 * s, -13, 100 * s, 26, 5);
  triangle(50 * s, -13, 50 * s, 13, 67 * s, 0);
  fill(100, 200, 255);
  circle(0, 0, 11);
  fill(col);
  triangle(-50 * s, -13, -60 * s, -22, -50 * s, -2);
  triangle(-50 * s, 13, -60 * s, 22, -50 * s, 2);
  
  if (v > 0.02) {
    fill(255, 200, 0, 220);
    noStroke();
    triangle(-50 * s, -7, -50 * s, 7, -67 * s - v * 25, 0);
    fill(255, 100, 0, 180);
    triangle(-50 * s, -4, -50 * s, 4, -60 * s - v * 18, 0);
  }
  
  if (v > 0.3) {
    stroke(255, 255, 255, 150);
    strokeWeight(2);
    for (let i = 0; i < 5; i++) {
      let offset = -80 - i * 20;
      line(offset, -5, offset - 15, -5);
      line(offset, 5, offset - 15, 5);
    }
  }
  pop();
}

function jam(x, y, r, waktu, lbl, col) {
  fill(20, 20, 40);
  stroke(col);
  strokeWeight(3);
  circle(x, y, r * 2);
  noStroke();
  fill(col);
  for (let i = 0; i < 12; i++) {
    let a = map(i, 0, 12, 0, TWO_PI) - HALF_PI;
    circle(x + cos(a) * (r - 4), y + sin(a) * (r - 4), 3);
  }
  let a = map(waktu % 12, 0, 12, 0, TWO_PI) - HALF_PI;
  stroke(col);
  strokeWeight(3);
  line(x, y, x + cos(a) * (r - 10), y + sin(a) * (r - 10));
  noStroke();
  fill(col);
  circle(x, y, 6);
  fill(255);
  textSize(11);
  textAlign(CENTER, CENTER);
  text(lbl, x, y + r + 13);
  text(waktu.toFixed(1) + 'h', x, y + r + 26);
}

// ===== EVENT =====
function mousePressed() {
  if (state === 'menu') {
    if (mouseX > width/2 - 220 && mouseX < width/2 - 20 && mouseY > 480 && mouseY < 545) {
      state = 'simulasi';
      rocketX = 0;
    } else if (mouseX > width/2 + 20 && mouseX < width/2 + 220 && mouseY > 480 && mouseY < 545) {
      state = 'quiz';
      no = 0;
      skor = 0;
      jawab = false;
    }
  } else if (state === 'simulasi') {
    if (mouseX > 25 && mouseX < 125 && mouseY > 595 && mouseY < 630) {
      v = 0;
      rocketX = 0;
    } else if (mouseX > width - 125 && mouseX < width - 25 && mouseY > 595 && mouseY < 630) {
      state = 'menu';
    }
  } else if (state === 'quiz') {
    if (no < soal.length && !jawab) {
      for (let i = 0; i < 4; i++) {
        let y = 205 + i * 70;
        if (mouseX > 70 && mouseX < width - 70 && mouseY > y && mouseY < y + 55) {
          ans = i;
          jawab = true;
          if (i === soal[no][2]) skor++;
          break;
        }
      }
    } else if (jawab && no < soal.length) {
      if (mouseX > width/2 - 80 && mouseX < width/2 + 80 && mouseY > 540 && mouseY < 585) {
        no++;
        jawab = false;
        ans = -1;
      }
    } else if (no >= soal.length) {
      if (mouseX > width/2 - 175 && mouseX < width/2 - 35 && mouseY > 460 && mouseY < 505) {
        no = 0;
        skor = 0;
        jawab = false;
      } else if (mouseX > width/2 + 35 && mouseX < width/2 + 175 && mouseY > 460 && mouseY < 505) {
        state = 'menu';
      }
    }
  }
}